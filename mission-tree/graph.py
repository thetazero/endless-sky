...
def draw_mission_node(mission: Mission, file: TextIOWrapper):
    file.write(
        f"{mission_fmt(mission)} [shape=box,fillcolor=lightgreen,style=filled]\n"
    )


def draw_payment(payment: Payment, file: TextIOWrapper):
    file.write(
        f"{payment_fmt(payment)} [shape=oval,fillcolor=lightpink,style=filled]\n"
    )
...
def visualize_missions(missions: list[Mission], output_path: str):
    ...
    with open(output_path, "w") as f:
        ...
        for mission in missions:
            if mission.to_offer.has:
                f.write(
                    f"{event_fmt(mission.to_offer.has.event)} -> {mission_fmt(mission)}\n"
                )
            if mission.on_complete:
                if mission.on_complete.payment:
                    f.write(
                        f"{mission_fmt(mission)} -> {payment_fmt(mission.on_complete.payment)}\n"
                    )
                for event in mission.on_complete.events:
